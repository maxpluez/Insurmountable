...
{                                                    
  ...
  {                                                // display():  Called once per frame of animation.  For each shape that you want to
    ...
    //IK Updates
    this.grips[0] = vec3(1+Math.sin(t), 7-Math.cos(t), 0); 
    let target = this.grips[0];
    let end_effector = this.robot.r_wrist.get_absolute_location().times(vec4(0,0,0,1)).to3();
    let anchor = this.robot.r_elbow.get_absolute_location().times(vec4(0,0,0,1)).to3();
    let delta = (end_effector.minus(anchor)).norm();
    // while(delta > 0.001) {
      delta = (end_effector.minus(anchor)).norm();
      end_effector = this.robot.r_wrist.get_absolute_location().times(vec4(0,0,0,1)).to3();
